      ...
      >
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={[styles.settingsBtn, styles.switchContainer]}>
            <Text style={styles.buttonText}>{t("settingsChangeLanguageEN")}</Text>
            <Switch
              value={isLanguageEnglish}
              onValueChange={() => setEnglishLanguage((previousState) => !previousState)}
              trackColor={{ false: "#999999", true: "#388FED" }}
              thumbColor={"#F9F9F9"}
            />
          </View>
          <View style={[styles.settingsBtn, styles.switchContainer]}>
            <Text style={styles.buttonText}>{t("settingsChangeLanguageBG")}</Text>
            <Switch
              value={!isLanguageEnglish}
              onValueChange={() => setEnglishLanguage((previousState) => !previousState)}
              trackColor={{ false: "#999999", true: "#388FED" }}
